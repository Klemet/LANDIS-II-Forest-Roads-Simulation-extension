...
namespace Landis.Extension.ForestRoadsSimulation
{
	public class MapManager
	{
		...
		public static double CostOfTransition(Site givenSite, Site otherSite)
		{
            ...
            // we add the smallest value for a double that is superior to 0. Is used in the case where we're moving from one road to the other without any cost :
			// In that case, adding this very small value will discriminate between a path with more roads or less roads.
            return (cost + double.Epsilon); 
		}
        ...
        /// includeArrivalSite is used when we encounter an endpath; at this moment, we want the path to start that includes the current cell in the dijkstra search,
		/// which is passed in arrivalSite.
        /// </summary>
        /// <returns>
        /// A list that beguins with the arrival site, and ends with the starting site.
        /// </returns>
        public static List<Site> FindPathToStart(Site startingSite, Site arrivalSite, Dictionary<Site, Site> predecessors, bool includeArrivalSite = false)
		{
